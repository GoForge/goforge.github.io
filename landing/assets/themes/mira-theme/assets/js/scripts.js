jQuery(document).ready(function() {
    ...
    */
	if(mira_scripts_js_vars.coming_soon_bg.images_number == 1) {
		jQuery('.coming-soon').backstretch(mira_scripts_js_vars.coming_soon_bg.images_src);
	}
	else if(mira_scripts_js_vars.coming_soon_bg.images_number > 1) {
		jQuery('.coming-soon').backstretch(mira_scripts_js_vars.coming_soon_bg.images_src, {duration: 3000, fade: 750});
	}

	/*
		Fullscreen backgrounds
	*/
	if(mira_scripts_js_vars.subscribe_bg.bg_type == 'image' && mira_scripts_js_vars.subscribe_bg.image_src != '') {
		jQuery('.subscribe-container').backstretch(mira_scripts_js_vars.subscribe_bg.image_src);
	}
	if(mira_scripts_js_vars.the_project_bg.bg_type == 'image' && mira_scripts_js_vars.the_project_bg.image_src != '') {
		jQuery('.about-container').backstretch(mira_scripts_js_vars.the_project_bg.image_src);
	}
	if(mira_scripts_js_vars.testimonials_bg.bg_type == 'image' && mira_scripts_js_vars.testimonials_bg.image_src != '') {
		jQuery('.testimonials-container').backstretch(mira_scripts_js_vars.testimonials_bg.image_src);
	}
	if(mira_scripts_js_vars.about_us_bg.bg_type == 'image' && mira_scripts_js_vars.about_us_bg.image_src != '') {
		jQuery('.whos-behind-container').backstretch(mira_scripts_js_vars.about_us_bg.image_src);
	}
	if(mira_scripts_js_vars.contact_bg.bg_type == 'image' && mira_scripts_js_vars.contact_bg.image_src != '') {
		jQuery('.contact-container').backstretch(mira_scripts_js_vars.contact_bg.image_src);
	}

    /*
        Countdown initializer
    */
    var countTo = "2015/05/31";
    ...
    */
    ...
    /*
	    Google maps
	*/
    var position = new google.maps.LatLng(mira_scripts_js_vars.contact_map.lat, mira_scripts_js_vars.contact_map.lng);
    ...
    */
    jQuery('.subscribe form').submit(function(e) {
    	e.preventDefault();        
        jQuery.post(mira_scripts_js_vars.ajaxurl,
	    			{
	    				// wp ajax action
	    				action : 'mira-ajax-subscribe-action',
	    				// vars
	    				email : jQuery('.subscribe form .subscribe-email').val(),
	    				// send the nonce along with the request
	    				nonce_subscribe : mira_scripts_js_vars.nonce_subscribe
	    			},
	    			function(response) {
	    				if(response.valid == 0) {
	...
	*/
	...
	jQuery('.contact-form form').submit(function(e) {
	    ...
	    jQuery.post(mira_scripts_js_vars.ajaxurl,
    			{
    				// wp ajax action
    				action : 'mira-ajax-contact-action',
    				// vars
    				...
    				// send the nonce along with the request
    				nonce_contact : mira_scripts_js_vars.nonce_contact
    			},
    			function(response) {
